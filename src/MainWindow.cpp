...
MainWindow::MainWindow() : m_hwnd(NULL), m_hTreeView(NULL), m_hTabControl(NULL), m_hStatusBar(NULL), m_treeWidth(250), m_isResizing(false), m_hImageList(NULL), m_hTabImageList(NULL)
{
}
    ...
    for (size_t i = 0; i < m_connections.size(); ++i) {
        ...
        if (!m_connections[i].group.empty()) {
            if (folders.find(m_connections[i].group) == folders.end()) {
                ...
                folders[m_connections[i].group] = TreeView_InsertItem(m_hTreeView, &tvis); TreeView_Expand(m_hTreeView, folders[m_connections[i].group], TVE_EXPAND);
            }
...
void MainWindow::LaunchSession(const Connection& conn)
{
    ...
    if (hPutty) {
        ...
        Session* s = new Session{ hPutty, pid, conn.name }; m_sessions.push_back(s);
...
void MainWindow::LaunchWinSCP(const Connection& conn)
{
    ...
    // Using /password switch which is cleaner than embedding in URL for special chars
